...
namespace USTC_CG
{
Freedraw::Freedraw(std::vector<float> x, std::vector<float> y)
{
    x_list=x;
    y_list=y;
}
// Draw the Freedraw using ImGui
void Freedraw::draw(const Config& config) const
    ...
    for (int i = 0; i < x_list.size()-1 ; i++)
    {
        ...
        /*draw_list->AddLine(
           ...
           draw_list->AddLine(
            ...
            not needed for freedraw*/

    }
}

void Freedraw::add_control_point(float x, float y) 
{
    //std::cout<<3<< std::endl;
...
void Freedraw::update(float x, float y) 
{
    //std::cout<<4<< std::endl;
    end_point_x_ = x;
    end_point_y_ = y;
