...
class Ellipse : public Shape
{
   public:
    ...
    // Initialize an ellipse with two points that define the bounding box of the ellipse
    Ellipse(
        float start_point_x,
        float start_point_y,
        float end_point_x,
    ...
    // Overrides Shape's update function to adjust the ellipse size during interaction
    void update(float x, float y) override;
    void add_control_point(float x, float y) override{(void)x;(void)y;};
   private:
    // Coordinates of the top-left and bottom-right corners of the ellipse's bounding box
